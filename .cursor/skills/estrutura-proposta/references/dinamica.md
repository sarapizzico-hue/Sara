...
Estudo do casco [ABC71 — deck unificado](https://relatorios.collieassociados.com/abc71-semana-4-deck-unificado/#slide-01). **Pegar o sistema de navegação. Não pegar o conteúdo da conta** (SWOT, personas, SEO, Drawflow, semanas de projeto).

Visual continua o da proposta: IBM Plex, Colli 1600×900, `red | dark | white`. Tipografia Montserrat / vinho da ABC71 **não entra**.
...
| **Divisor de ato** | Subcapa: número grande + kicker + título + trilho do que vem | Um slide `divider` **antes** de Dados, Produto e Decisão (EC 13–18 e STEP 14). Combo 7 slides: só se o brief pedir — senão a pílula basta |
...
- Casco com **vários iframes** (44 slides em 4 arquivos) — overkill para proposta de 7–18. **Um único HTML** na raiz
- Paleta vinho, grid de fundo, Montserrat, JetBrains Mono
- SWOT / TOWS / comitê de compra como arco de proposta (isso é diagnóstico de projeto, não venda)
...
function goTo(index) {
  // 1. ativa o slide
  // 2. atualiza counter 01 / N  (IBM Plex Mono)
...
Âncora de **conteúdo/visual**: Muller / G6. Âncora de **dinâmica de sala**: este arquivo (ABC71 casco).
